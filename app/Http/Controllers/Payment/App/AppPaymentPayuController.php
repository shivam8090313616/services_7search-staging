...
class AppPaymentPayuController extends Controller
{
    public function payment_payu(Request $request)
    {
      ...
      $validator = Validator::make(
        ...
        if (empty($users)) {
            ...
            else
            {
              ...
              return view('payment.apppayu',$data);
    }
    public function response(Request $request)
    {
        ...
        if($status == 'failure')
        {
          	$request_for  = $_POST['udf2'];
            $trxid  = $_POST['udf1'];
            $remark  = $_POST['error_Message'];
            ...
            if($transac->update())
            {
                 ...
                 $msg = "PayU|$email|Failed|$paymentid|$amount|usd";
        ...
        else if($status == 'success')
        {
            ...
            if($statu == 0)
            {
                ...
                if($transac->update())
                {
                    ...
                    $transaclog->remark            = 'Amount added to wallet succefully ! - Payu';
                    if($transaclog->save())
                    {
                    ...
                    paymentSuccessMail($subjects, $fullname, $emailname, $phone, $addressline1, $addressline2, $city, $state, $country, $createdat, $useridas, $transactionid, $paymentmode, $amount, $paybleamt, $fee, $gst, $remark ,$subtotal);
                      ...
                      /* Coupon Section  Transaction Log Save  */
                        ...
                        $remark = $transac->remark;
                        $subjects = "Fund Added Successfully";
                        $mailsentdetals = ['subject' =>$subjects, 'full_name'=>$fullname, 'user_id'=>$useridas,'transaction_id'=>$transactionid, 'payment_mode'=>$paymentmode, 'amount'=>$amount, 'payble_amt'=>$paybleamt, 'fee'=>$fee, 'gst'=>$gst, 'remark'=>$remark];
                        $mailTo = [$email];
                        /* Mail Section */
                        Mail::to($mailTo)->send(new TransactionMail($mailsentdetals));                         
                        $msg = "PayU|$email|Success|$paymentid|$amount|usd";
                        $msg1 = base64_encode($msg);
                        if($request_for == 'App')
                        {
                          echo '<script>
