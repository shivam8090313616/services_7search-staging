...
{
    public function handleVerification(string $uid, ?string $method = null, string $type): array
    {
        try {
            ...
            if ($method === 'sendMail') {
                ...
                $mailStatus = $this->sendVerificationEmail($user, $type);
    ...
    protected function sendVerificationEmail(User $user, string $type): array
    {
        try {
            if ($user->status == 1) {
                return [
        ...
        if (Cache::has($cacheKey)) {
            ...
            $verificationLink = $type === 'adv'
            ? "http://localhost:3000/verify-mail/advertiser"
            : "http://localhost:3000/verify-mail/publisher";
