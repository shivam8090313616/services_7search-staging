...
class AuthController extends Controller
{
    ...
    public function emailVerify(Request $request)
    {
        ...
        $response = $this->accVerifiedService->verifyAccount(
            $validated['userId'],
            $validated['token']
        );
        ...
        if (!$user) {
            ...
            if ($user->user_type == 2) {
                ...
                if ($loginactivity->save()) {
                    ...
                    $mail_verified = User::where('uid', $user->uid)->value('mail_verified');
                    ...
                    $return['mail_verified'] = $mail_verified;
    ...
    {
        $user = User::select('first_name', 'last_name', 'messenger_name', 'messenger_type', 'email', 'phone', 'phonecode', 'address_line1', 'address_line2', 'city', 'state', 'country', 'wallet', 'profile_lock', 'del_request', 'user_type','mail_verified')
            ->where('uid', $uid)
            ->first();
    ...
    public function tokenValidate(Request $request)
    {
        ...
        $emp = DB::table('admins')
            ...
            ->where('admins.emp_id', $request->empid)
            ->first();
        if ($emp) {
        ...
        if (!$user || !Hash::check($request->password, $user->password)) {
            ...
            if ($user->user_type >= '2') {
                ...
                if ($loginactivity->save()) {
                    ...
                    $return['mail_verified'] = $user->user_type;
