        ...
        if ($camp->update())
        {
            ...
            elseif ($camp_status == 2)
            {
                 ...
                 $maximunBid = Campaign::where('website_category', $camp->website_category)
                    ...
                    ->first();
                if($camp->cpc_amt >=  $maximunBid->cpc_amt){
                    $noti_desc = 'A new bid has been placed for the campaign - '. $camp->campaign_id.', Category Name - '. $camp->cat_name.', Bid Amount - $'. $camp->cpc_amt.' ';                    $activitylog = new Activitylog();
                    $activitylog->uid = 'Admin';
                    $activitylog->type = 'active';
                    $activitylog->description =  $noti_desc;
                    $activitylog->status = '1';
            ...
            $return['data'] = $camp;
            $return['message'] = 'Updated Successfully';
            if($user->trash == 0 && $user->user_type != 2){ 
    ...
    public function topbidCamplist(Request $request)
     {
        ...
        $categorylist = Category::select('id', 'cat_name')->orderBy('cat_name', 'asc')->get();
        $data = [];
