...
class AuthController extends Controller
{
    ...
    public function verify(Request $request)
    {
        ...
        if (!$user || !$request->token || $user->verify_code === null || $request->token !== $user->verify_code) {
            return response()->json([
        ...
        $user->update([
            'ac_verified' => 1,
            'email_verified_at' => now(), // Use Laravel's helper for current timestamp
            'verify_code' => null, // Clear the token after verification
        ]);
    }
