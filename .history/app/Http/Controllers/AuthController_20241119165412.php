...
class AuthController extends Controller
{
    ...
    public function checkVerification(Request $request)
    {
        ...
        try {
            // Use the AccVerifiedService to handle verification or send email
            $response = $this->accVerifiedService->handleVerification(
                $validated['uid'],
                $validated['method'] ?? null // Pass method or null if it's not provided
            );
    ...
    public function verify(Request $request)
    {
        ...
        // If user not found or account already verified, return an error with code 402
        if (!$user || $user->ac_verified == 1) {
            return response()->json([
                'message' => 'This link has expired or is invalid!',
                'code' => 402 // Expired or Invalid link
            ], 402); 
        }
        
        // Update user's verification status to 1
        $user->update(['ac_verified' => 1]);
