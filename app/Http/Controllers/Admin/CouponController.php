...
class CouponController extends Controller
{
    ...
    public function delete_coupon(Request $request)
    {
        $id = $request->id;
        $coupon = Coupon::where('id', $id)->first();
        return response()->json(["fgxh"]);
        if ($coupon->coupon_code == 'REF_REWARD') {
            $return['code'] = 101;
            $return['message'] = 'Something went wrong !';
            return json_encode($return, JSON_NUMERIC_CHECK);
        }
    ...
    public function refAdvUser(Request $request)
    {
        $uid = $request->uid;
        $amount = $request->coupon_amt;
        $userTransCount = Transaction::where('status', 1)->where('advertiser_code', $uid)->count();
        $refUser = User::where('uid', $uid)->first();
        $coupon = Coupon::where('coupon_code', 'REF_REWARD')->where('status', 1)->where('trash', 0)->first();
        ...
        if ($userTransCount == 0 && $refUser->referal_code != null) {
            $getcalampdata = getCouponCal($uid, $coupon->coupon_code, $amount, $coupon->coupon_id);
            return json_encode($getcalampdata, JSON_NUMERIC_CHECK);
        }

        return response()->json([
            'message' => 'Not eligible for ReferralUser coupon',
            'applicable' => 'false',
            'coupon_code' => null,
            'code' => '200'
        ], 200);
